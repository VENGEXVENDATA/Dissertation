...
def assign_crisis_phase(window_date, config):
    ...
    pre_days = config.get('pre_crisis_window_days', 63)
...
def main():
    ...
    print("\n" + "="*75)
    print("   EMPIRICAL VERIFICATION: MEAN NETWORK STABILITY ($ARI$) BY MACRO REGIME")
    print("="*75)
    
    summary = df.groupby(['regime', 'market'])['ari_stability'].mean().unstack(level=1)
